 ...
 * GET add product
 */
router.get('/add-products', isAdmin, (req, res) => {
	const {
		title,
		desc,
		dummyPrice,
 ...
 * GET edit product
 */
router.get('/edit-products/:_id', isAdmin, (req, res) => {
	Category.find((err, categories) => {
		if (err) return console.log(err);
 ...
 * GET delete product
 */
router.get('/delete-product/:_id', isAdmin, (req, res) => {
	Product.findByIdAndDelete(req.params._id, function (err, data) {
		...
		fs.unlink(path, err => {
			if (path === 'public/uploads') {
				...
				return console.log(`dont solve this error ${err}`);
			} else {
